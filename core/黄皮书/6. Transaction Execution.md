...
交易执行是以太坊协议中最复杂的部分：它定义了状态转换函数 $\Upsilon$。我们假设任何被执行的交易都已经通过了初步的内在有效性测试。这些测试包括：

1. 交易是格式良好的 RLP 编码，且没有额外的尾部字节；
2. 交易签名是有效的；
3. 交易的 nonce 是有效的（即等于发送方账户当前的 nonce）；
4. 发送方账户未部署任何合约代码（参见 Feist 等人提出的 [EIP-3607, 2021]）；
5. gas 限额不小于交易所需的固有 gas（$g_0$）；
6. 发送方账户余额不少于预支付中所需的费用（$v_0$）；
7. 对于类型 2 的交易，`maxFeePerGas`（记作 $T_m$），或者对于类型 0 和类型 1 的交易，`gasPrice`（记作 $T_p$）必须大于等于区块的基本费用（$H_f$）；
8. 对于类型 2 的交易，`maxPriorityFeePerGas`（记作 $T_f$）不能大于 `maxFeePerGas`（$T_m$）。
...
- $\Upsilon_g$：评估交易执行所使用的 gas 数量；
- $\Upsilon_l$：评估交易执行过程中产生的日志项；
- $\Upsilon_z$：评估交易执行的状态码（成功或失败）。
...
## 6.3 交易有效性判断（Transaction Validity）
...
- **倒数第二个条件**（$n \le 49152$）确保在合约创建交易中，初始化代码不得超过 49152 字节。
- **最后一个条件**限制了交易使用的 gas 总量（已使用 + 当前交易）不能超过区块 gas 限额。

此外，使用一种简化记号，我们假设当 $\sigma[S(T)] = \varnothing$ 时，也即账户不存在时：
...
## 6.5 检查点状态（Checkpoint State）与执行过程
...
即，在扣除固有 gas（intrinsic gas）之后的剩余 gas，用于实际执行合约或消息调用操作。

> 在上述定义中，我们使用 $\Lambda_4$ 和 $\Theta_4$ 表示仅采集函数返回值的前四项。第五项是消息调用返回的字节数组（byte array），在交易评估过程中不会使用。

---

## 6.6 gas 退款计算（Refund Calculation）
...
## 6.9 交易结果函数（Transaction Result Functions）
