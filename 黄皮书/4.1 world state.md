...
尽管世界状态本身不会直接存储在 **区块链（blockchain）** 上，但实现时会使用 **Modified Merkle Patricia Trie, MPT** 来维护该映射。该 **Trie** 需要一个简单的 **数据库后端**，该数据库维护 **byte array 到byte array** 的映射，我们称这个底层数据库为 **状态数据库（state database）**。
